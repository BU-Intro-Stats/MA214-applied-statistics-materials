...
#
# Blanks marked ____ are for you to fill in. Work through the worksheet in order.
...
chains <- lapply(1:4, function(i)
  run_mcmc(M,
           step_b0  = ____,      # try 0.1 first, then fix it
           step_b1  = ____,      # try 0.0001 first, then fix it
           step_sig = ____,      # try 0.05 first, then fix it
...
burn <- ____          # read it off the trace plots
...
chains_sharp <- lapply(1:4, function(i)
  run_mcmc(M, step_b0 = ____, step_b1 = ____, step_sig = ____,
           start_b0 = start_b0s[i], start_b1 = start_b1s[i],
           start_sig = start_sigs[i], seed = 7 + i) |>
    mutate(chain = factor(i)))
