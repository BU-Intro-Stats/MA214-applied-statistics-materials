...
# In-class activity
# COMPLETED VERSION (instructor reference)
...
n    <- length(sbp_data)
xbar <- mean(sbp_data)          # 126
SS   <- sum((sbp_data - xbar)^2)  # 668
...
ggplot(grid, aes(x = mu, y = sigma, z = post)) +
  ...
  theme_minimal()
# The posterior concentrates around mu ~ 126, sigma ~ 6-7.
...
# mu converges by K~11; sigma takes until K~15-21.
# With 2 parameters, K=21 requires 441 points. With 10 parameters,
# K=21 would need 21^10 ~ 16 trillion -- impossible.
...
# mu: mean ~ 125.9, sd ~ 1.5
# sigma: mean ~ 6.8, sd ~ 1.2
...
# Using step = 0.5 (smaller than the "good" step of 1.5) so the chain
# takes longer to walk from (140, 25) to the posterior, making burn-in
# clearly visible in the trace plots.
set.seed(214)
...
bind_rows(
  ...
  theme_minimal()
# mu drifts down from 140 toward ~126; sigma drifts down from 25 toward ~7.
# The initial transient (burn-in) is clearly visible over the first
...
# Without burn-in removal, the mean is pulled toward the starting value.
# After dropping ~500, the summaries stabilize.
...
# With only 1500 post-burn-in samples (and a slow-mixing step = 0.5 chain),
# the MCMC estimates may noticeably differ from the grid — especially for sigma.

paste("Acceptance rate:", sprintf("%.4f", result$acc_rate))
# With step = 0.5, acceptance rate is high (~0.8).
# High acceptance = small steps = slow exploration = need more iterations.
...
# With 9500 post-burn-in samples, MCMC estimates are much closer to grid.
# Chain length matters because each MCMC sample is correlated with its
# neighbors — a longer chain gives more effectively independent draws.
...
# Too small: ~98%, Good: ~40-50%, Too large: ~2%
...
bind_rows(lapply(seq_along(chains_good), function(i) {
  ...
  theme_minimal()
# All four chains converge to the same region within ~100 iterations.
...
bind_rows(lapply(seq_along(chains_bad), function(i) {
  ...
  labs(x = "Iteration", y = expression(mu), color = "Start") +
  theme_minimal()
# Chains stay near starting values -- clearly not converged.
...
# R-hat >> 1 for both parameters (mu ~ 5, sigma ~ 6)
